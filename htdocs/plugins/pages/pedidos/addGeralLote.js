...
var paginaOperacao = 'tabLote';

var codigoTab = document.getElementById("loteMaterial").value;
...
var paginaOperacao = 'tabLocal';

var codigoTab = document.getElementById("localMaterial").value;
...
function confirmaExpedicao() {
    ...
    else if (!document.getElementById("observacoesExpedicao").value) {

        Toast.fire({
            icon: 'error',
            title: 'Erro ao salvar o orçamento, nenhuma observação da expedição foi informada.'
        })
...
function adicionarProduto(operacao) {
    ...
    var loteMaterial = document.getElementById('loteMaterial').value;
    var localMaterial = document.getElementById('localMaterial').value;
    var total = 0;
    ...
    if (!codigoMaterial | codigoMaterial <= 0) {
        ...
        if (!quantidadeMaterial | quantidadeMaterial <= 0) {
            ...
            if (!unitarioMaterial | unitarioMaterial <= 0) {
                ...
                if (!loteMaterial | loteMaterial <= 0) {

                    Toast.fire({
                        icon: 'error',
                        title: 'Lote não informado, por favor informe um lote.'
                    });
                    ...
                    if (!localMaterial | localMaterial <= 0) {

                        Toast.fire({
                            icon: 'error',
                            title: 'Local não informado, por favor informe um local.'
                        });
...
function incluirLoteLocal(intValue, intValue2) {

   document.getElementById('localMaterial').value = intValue;
   document.getElementById('loteMaterial').value = intValue2;

   tabLote();
   tabLocal();

   $('#escolherLoteLocal').modal('hide');
